...
export class CoordsToModel {
    ...
    constructor(data) {
        ...
        this.attack = this.r(((data.coords.attack.x - data.coords.begin.x) / data.coords.travelUnit) / 10);
        if (data.staccato) {
            this.release = this.r(((data.coords.release.x - data.coords.attack.x) / data.coords.travelUnit) / 10);
        } else {
            this.decay = this.r(((data.coords.decay.x - data.coords.attack.x) / data.coords.travelUnit) / 10);
            this.sustain = this.r(FLOOR - ((FLOOR - CIEL) / 10) * (data.coords.sustain.y / 10));
            this.release = this.r(((data.coords.release.x - data.coords.sustain.x) / data.coords.travelUnit) / 10);
        }
