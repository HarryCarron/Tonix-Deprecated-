...
export class PartialsComponent {
  ...
  clickIsWithinContainer(e): boolean {
    ...
    if (
      e.clientX >
      this.partialsSelectorBounding.x + this.partialsSelectorBounding.width
    ) {
      return false;
    }
    ...
    if (
      e.clientY >
      this.partialsSelectorBounding.y + +this.partialsSelectorBounding.height
    ) {
      return false;
    }
  ...
  partialEntryActive({ clientX: x, clientY: y }) {
    if (this.partialEntryMoving) {
      ...
      const hoveredPartial = Math.floor(
        newx / (this.partialsSelectorBounding.width / this.partials.length)
      );
      const newValue = parseFloat((newy / 100).toFixed(2));

      if (
        !!this.partials[hoveredPartial] ||
  ...
  randomisePartials() {
    ...
    this.partials = Array.from({ length: r(32) }).map((a) => r(10) / 10);
